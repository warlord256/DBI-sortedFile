...
int Record :: ComposeRecord (Type type, int& intRes, double& doubleRes) {
	...
	if(type == Int) {
		*((int*) &(recSpace[currentPosInRec])) = intRes;
		...
		*((double *) &(recSpace[currentPosInRec])) = doubleRes;
...
void Record :: MergeRecords(Record *left, Record *right) {
	int leftCount = left->NumberOfAtts();
	int rightCount  = right->NumberOfAtts();
	int atts[leftCount+rightCount];
	...
	MergeRecords(left, right, leftCount, rightCount, atts, leftCount+rightCount, leftCount);
}
...
void Record :: WriteToFile (FILE *file, Schema *schema) {
