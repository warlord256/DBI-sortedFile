...
void q6 () {
	...
	GroupBy G;
		...
		Function func;
			...
			OrderMaker grp_order (&join_sch, 1, &s_nationkey);
	G.Use_n_Pages (1);
...
*/
	...
	Join J;
		...
		Attribute ps_supplycost = {"ps_supplycost", Double};
		Attribute joinatt[] = {IA,SA,SA,SA,SA,IA,SA,DA,SA,IA,ps_suppkey,IA,ps_supplycost,SA};
		Schema join_sch ("join_sch", outAtts, joinatt);
	...
	Join J2;
		...
		outAtts = sAtts + outAtts;
		Attribute joinatt2[] = {IA,SA,SA,IA,SA,DA,SA,IA,SA,SA,SA,SA,IA,SA,DA,SA,IA,ps_suppkey,IA,ps_supplycost,SA};
		Schema join_sch_2 ("join_2_sch", outAtts, joinatt2);

	J2.Use_n_Pages(100);
	

	// On _s_p_ps:
	// 	S(ps_supplycost) => __s_p_ps
	Sum S;
		Pipe __s_p_ps(1);
		Function func;
		CNF cnf_sum_s;
		get_cnf("(ps_supplycost)", &join_sch_2, func);
	...
	// 	W(__s_p_ps)
	WriteOut W;
		char *fwpath = "ps.s.res";
		FILE *writefile = fopen (fwpath, "w");
		Schema out_sch("sum", 1, &DA);
	
	SF_s.Run (dbf_s, _s, cnf_s, lit_s);
	SF_p.Run (dbf_p, _p, cnf_p, lit_p);
	SF_ps.Run (dbf_ps, _ps, cnf_ps, lit_ps);
	J.Run(_p, _ps, _p_ps, cnf_p_ps, lit_p_ps);
	J2.Run(_s, _p_ps, _s_p_ps, cnf_s_p_ps, lit_s_p_ps);
	S.Run(_s_p_ps, __s_p_ps, func);
	W.Run(__s_p_ps, writefile, out_sch);
	// W.Run(_s_p_ps, writefile, join_sch_2);
...
*/
	...
	cout << "query 8\n";
// 	SF (l_returnflag = 'R' and ...) => _l
	char *pred_l = "(l_returnflag = 'R') AND (l_discount < 0.04 OR l_shipmode = 'MAIL')";
	init_SF_li (pred_l, 100);
	
	SF_li.Use_n_Pages(128);
...
// 		W (__l)
	...
	char *fwpath = "li.res";
	FILE *writefile = fopen (fwpath, "w");
	Attribute l_orderkey = {"l_orderkey", Int};
	Attribute l_partkey = {"l_partkey", Int};
	Attribute l_suppkey = {"l_suppkey", Int};
	Attribute p_attr[] = {l_orderkey, l_partkey, l_suppkey};
	Schema out_sch("select", 3, p_attr);
	
	SF_li.Run(dbf_li, _li, cnf_li, lit_li);
	P.Run(_li, __l, toKeep, liAtts, 3);
	W.Run(__l, writefile, out_sch);
